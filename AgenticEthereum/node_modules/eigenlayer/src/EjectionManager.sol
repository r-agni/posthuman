...
contract EjectionManager is IEjectionManager, OwnableUpgradeable {
    ...
    function ejectOperators(bytes32[][] memory _operatorIds) external {
        ...
        for (uint i = 0; i < _operatorIds.length; ++i) {
            ...
            for (uint8 j = 0; j < _operatorIds[i].length; ++j) {
                uint256 operatorStake = stakeRegistry.getCurrentStake(
                    _operatorIds[i][j],
                    quorumNumber
                );
    ...
    function amountEjectableForQuorum(
        ...
        uint256 cutoffTime = block.timestamp -
            quorumEjectionParams[_quorumNumber].rateLimitWindow;
        uint256 totalEjectable = (uint256(
            quorumEjectionParams[_quorumNumber].ejectableStakePercent
        ) * uint256(stakeRegistry.getCurrentTotalStake(_quorumNumber))) /
        ...
        while (stakeEjectedForQuorum[_quorumNumber][i].timestamp > cutoffTime) {
            totalEjected += stakeEjectedForQuorum[_quorumNumber][i]
                .stakeEjected;
            if (i == 0) {
                break;
            } else {
